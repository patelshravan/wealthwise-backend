...
const mailGenerator = new Mailgen({
    ...
    product: {
        name: 'Wealthwise',
        link: '#',
        logo: 'https://res.cloudinary.com/patelshravan/image/upload/v1743930659/wealthwiselogo_i7b5wr.png',
    }
});
...
const sendOtpOnMail = async (email, name, otp) => {
    ...
    try {
        ...
        console.error('❌ OTP email sending failed:', error.response?.body || error.message);
    }
};
...
const sendPasswordResetOtpOnMail = async (email, name, otp) => {
    ...
    try {
        ...
        console.error('❌ Password reset OTP email sending failed:', error.response?.body || error.message);
    }
};
