...
const createSavings = catchAsync(async (req, res) => {
  ...
  res
    ...
    .json({ message: CONSTANTS.SAVINGS_CREATE, data: result.data });
});

const getSavings = catchAsync(async (req, res) => {
  const { search = "", page = 1, limit = 10 } = req.query;
  const result = await savingsService.getSavingsByUser(
    req.user._id,
    search,
    page,
    limit
  ...
  res
    .status(result.status)
    .json({ message: CONSTANTS.SAVINGS_FETCH, data: result.data });
});
...
const updateSavings = catchAsync(async (req, res) => {
  ...
  res
    .status(result.status)
    .json({ message: CONSTANTS.SAVINGS_UPDATE, data: result.data });
});
